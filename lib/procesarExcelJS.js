...
function procesarDatos(rows, lanesNS, lanesSN, intervals, columnMap = {}) {
  ...
  // 🔹 Obtener zonas a procesar: usar seleccionadas si existen, si no, tomar todas las del archivo
...
function procesarZona(df, zona, intervalos) {
  ...
  for (const fecha of fechas) {
    ...
    const totalParaFecha = Object.values(porHora).reduce((a, b) => a + b, 0);
    ...
    // DEBUG: Mostrar suma por fecha
    console.log(`[DEBUG] Zona ${zona}, Fecha ${fecha}: Total=${Math.round(totalParaFecha)}, Registros=${registrosFecha.length}, Avg=${(totalParaFecha / registrosFecha.length).toFixed(1)}`);
  }

  console.log(`Resultados generados para zona ${zona}:`, resultados.length);
  const totalZona = resultados.reduce((a, r) => a + r.Total_vehiculos, 0);
  console.log(`[DEBUG] Total vehículos para zona ${zona}: ${totalZona}`);
  return resultados;
}
